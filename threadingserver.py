...
class ThreadedTCPRequestHandler(SocketServer.BaseRequestHandler): 
    ...
    def handle(self): 
        ...
        logger.info("You have a new client on port %s" % (self.client_address[1]))
        data = self.request.recv(1024) 
        logger.debug("Data incomming: %s" % (data))
    ...
    def finish(self):
        logger.info( "Finish connection with a client on port %s" % (self.client_address[1]))
